...
from apps.assessment_admin.adminforms.candidateassessmentreport_form import CandidateAssessmentReportForm
...
from apps.assessment_candidate.services.candidateanswer_service import CandidateAnswerService
...
class ReportCandidateAssessmentView(View):
    ...
    def get(self,request,*args,**kwargs):
        ...
        context={
            'pagetitle': 'Candidate Assessment Report','candassessreportform':self._candassessform
                                    }
    ...
    def post(self, request, *args, **kwargs):        
        ...
        try:
            ...
            if assessment_fromdate and assessment_lastdate:
                assessmentconducted_list=self._assessmentassignservice.get_assessmentconduct_by_filter(assessment_fromdate=assessment_fromdate,
...
candidate_mailid=candidate_mailid
                ...
                if assessmentconducted_list:
                    for assessmentconduct in assessmentconducted_list:
                        ...
                        correct = assessmentconduct.candidate_answers.filter(score=1.0).count()
                        percentage = round((correct / total) * 100) if total > 0 else 0
                        assessmentconduct.score=percentage
                        assessmentconduct.signed_id=signing.dumps({'id':assessmentconduct.id},salt='assessmentconduct-salt')
        ...
        context = {
            'pagetitle': 'Candidate Assessment Report','candassessreportform':post_form,'page_obj':self.page_obj
    ...
    def get(self,request,*args,**kwargs):
        ...
        try:
            ...
            if selected_assessmentconductid_decoded:
                    ...
                    current_assessmentconduct=self._assessmentassignservice.get_assessmentconduct_by_id(recordid=selected_assessmentconductid_decoded)
                    self.current_assessment_candidateanswer=self._candidateanswerservice.read_candidate_questions(current_assessmentconduct.id)

                    total = current_assessmentconduct.candidate_answers.count()
                    correct = current_assessmentconduct.candidate_answers.filter(score=1.0).count()
                    assessment_percentage = round((correct / total) * 100) if total > 0 else 0
                    ...
                    for cand_answer in self.current_assessment_candidateanswer:
                        ...
                        if cand_answer.selected_option:
                            cand_opt=cand_answer.question.options.filter(id=getattr(cand_answer.selected_option,"id")).first()
                        candidate_Ind_data['candidateoption']=cand_opt.option_text if cand_opt else None
                        candidate_Ind_data['answerstatus']=None
                        if cand_opt:
                            if cand_opt.id==cand_answer.question.options.filter(is_correct=True).first().id:
        ...
        context={
            'pagetitle': 'Candidate Assessment Report','candassessreportform':self._candassessform,'current_assessment_candidateanswer':candidate_assessment_data,
'assessment_percentage':assessment_percentage,'current_assessmentconduct':current_assessmentconduct
                                    }
        return render(request, self.template_name, context)
...
class ReportDownloadPDF(View):
    def post(self, request, *args, **kwargs):
        try:
            ...
            ViewError(message="ReportDownloadPDF: Failed to generate PDF", original_exception=Ex, log=True)
            return HttpResponse("Error generating PDF", status=500)