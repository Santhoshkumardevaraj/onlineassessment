    ...
    def get_assessments_assigned_for_candidate(self, candidate_profile, currentuser):
        ...
        if self.assigned_assessment:
            ...
            for item in self.assigned_assessment:
                item.signed_id = signing.dumps(
                    {"id": item.id}, salt="assessmentconduct-salt"
                )
        ...
        try:
            ...
            if cand_loginid and cand_loginid != "":
                candidate_profile = self._user_service.get_user_list_filtered(
                    loginid=cand_loginid, role="Candidate"
                ).first()
                ...
                if candidate_profile and candidate_profile.username:
                    ...
                    if self.assessment_report_list:
                        ...
                        paginator = Paginator(
                            self.assessment_report_list, self.paginate_by
                        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if ActionName and ActionName == "assignassessment":
            try:
                ...
                assessment = self._assessmentservice.get_assessment_by_id(
                    recordid=selected_recordid_decoded
                )
                if selected_assessmentdate:
                    if assessment:
                        check_assessmentassigned = (
                            self._assessmentassignservice.check_assessmentconduct(
                                candidate=candidate_profile,
                                assessment=assessment,
                                assessment_date=selected_assessmentdate,
                                currentuser=request.user,
                            )
                        )
                        if check_assessmentassigned:
                            messages.warning(
                                request,
                                "Candidate already have another assessment pending",
                            )
                        else:
                            self._assessmentassignservice.create_assessmentconduct(
                                candidate=candidate_profile,
                                assessment=assessment,
                                assessment_date=selected_assessmentdate,
                                currentuser=request.user,
                            ...
                            messages.success(
                                request, "Assessment assigned successfully"
                            )
                    else:
            ...
            if form.is_valid():
                try:
                    ...
                    if updated_assessment_data:
                        assessment = self._assessmentservice.update_assessment(
            ...
            try:
                delete_assessment = self._assessmentservice.get_assessment_by_id(
                    recordid=selected_recordid_decoded
                )
                if delete_assessment:
                    assessment = self._assessmentservice.update_assessment_remove(
            ...
            if form.is_valid():
                try:
                    assessment = self._assessmentservice.create_assessment(
                        form.cleaned_data, currentuser=request.user
                    )
                    if assessment:
            ...
            if form.is_valid():
                try:
                    self.assessment_report_list = (
                        self._assessmentservice.search_assessment(
                            form.cleaned_data, currentuser=request.user
                        )
                    )
