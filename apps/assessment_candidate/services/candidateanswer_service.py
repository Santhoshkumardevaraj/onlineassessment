...
from apps.commonapp.repositories.candidateanswer_repository import \
...
class CandidateAnswerService:
    ...
    def create_candidateanswer(
        ...
        try:
            ...
            cand_canswer = self._candidateanswer_repo.create_candidateanswer(
            ...
            raise ServiceError(
                message="CandidateAnswerService: Failed to get data:",
    ...
    def read_candidate_questions(self, assessmentconductid):
        try:
            assessment_Conduct_repo = AssessmentConductRepository()
            assessmentconduct = assessment_Conduct_repo.get_assessmentconduct_by_id(
                recordid=assessmentconductid
            )
            candidate_answer_list = self._candidateanswer_repo.read_candidate_questions(
                assessmentconduct=assessmentconduct
            )
    ...
    def update_assessment(self, cleaned_data, currentuser):
        try:
            ...
            assessment = self._assessment_repo.update_assessment(
    ...
    def update_assessment_remove(self, cleaned_data, currentuser):
        try:
            model_data = cleaned_data
            assessment = self._assessment_repo.update_assessment_remove(
                assessment_data=model_data, currentuser=currentuser
            )
            return assessment
        except Exception as Ex:
    ...
    ### QUESTION FUNCTIONS ###
    def create_question(
        self, curr_assessment=None, cleaned_data=None, currentuser=None
    ):
        try:
            ...
            for loopcount in range(0, len(List_options)):
                option_model_data = {
                    "option": List_options[loopcount],
                    "is_correct": loopcount == correct_option,
                }
                List_model_options.append(option_model_data)

            Question_model_data = {"text": text, "question_type": question_type}

            assessment = self._assessment_repo.create_question(
                assessment=curr_assessment,
                Question_model_data=Question_model_data,
                List_model_options=List_model_options,
                currentuser=currentuser,
            )
    ...
    def update_question_remove(self, cleaned_data, currentuser):
        try:
            model_data = cleaned_data
            assessment = self._assessment_repo.update_question_remove(
                question_data=model_data, currentuser=currentuser
            )
            return assessment
        except Exception as Ex:
            raise ServiceError(
    ...
    def update_question_and_options(self, cleaned_data, question, currentuser):
        try:
            model_data = cleaned_data
            question = self._assessment_repo.update_question_and_options(
                cleaned_data, question, currentuser
            )
            return question
        except Exception as Ex:
            raise ServiceError(
