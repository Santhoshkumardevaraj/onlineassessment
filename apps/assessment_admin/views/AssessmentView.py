    ...
    def get(self,request,*args,**kwargs):
        try:
            ...
            if self.assessment_report_list:
                ...
                ViewError(message="CreateAssessmentView: Failed on get method:", original_exception=Ex,log=True)
                messages.error(request, "Page Error Please Escalate")

        context={
            'pagetitle':'Create Assessment','AssessmentForm':_assessmentform,'AssessmentSearchForm':_searchform,'AssessmentList':self.assessment_report_list,'page_obj':page_obj,
    ...
    def post(self, request, *args, **kwargs):
        ...
        if ActionName and ActionName=="editassessment":
            try:
                ...
                assessment=self._service.get_assessment_by_id(recordid=selected_recordid_decoded)
                if assessment:
            ...
            if form.is_valid():
                try:
                    ...
                    if updated_assessment_data:
                        assessment = self._service.update_assessment(form.cleaned_data,currentuser=request.user)
                        if assessment:
                            form = self.form_class(instance=assessment)
                            self.form_current_action="UpdateAssessment"
            ...
            try:
                delete_assessment=self._service.get_assessment_by_id(recordid=selected_recordid_decoded)                 
                if delete_assessment:
                    assessment = self._service.update_assessment_remove(delete_assessment,currentuser=request.user)
                    if assessment:
            ...
            if form.is_valid():
                try:
                    assessment = self._service.create_assessment(form.cleaned_data,currentuser=request.user)
                    if assessment:
            ...
            if form.is_valid():
                try:
                    self.assessment_report_list = self._service.search_assessment(form.cleaned_data,currentuser=request.user)
        ...
        # Code to redirect control to QUESTION add or edit page
            ...
            question_edit_assessment=self._service.get_assessment_by_id(recordid=selected_recordid_decoded)          
            ...
            try:                 
                ...
                ViewError(message="CreateAssessmentView: Failed to add question:", original_exception=Ex,extra=None,log=True)
                messages.error(request, "Data Save Failed")    
