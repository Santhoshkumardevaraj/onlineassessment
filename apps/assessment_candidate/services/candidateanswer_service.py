...
class CandidateAnswerService:
    ...
    def create_candidateanswer(self,assessmentconduct=None,candidate=None,assessment=None,questions=None,currentuser=None):
        try:
            ...
            cand_canswer = self._candidateanswer_repo.create_candidateanswer(assessmentconduct=assessmentconduct,candidate=candidate,assessment=assessment,questions=questions,currentuser=currentuser)

            return cand_canswer
        except Exception as Ex:
            raise ServiceError(message="CandidateAnswerService: Failed to get data:", original_exception=Ex,log=True)

    def read_candidate_questions(self,assessmentconductid):
        try:
            assessment_Conduct_repo=AssessmentConductRepository()
            assessmentconduct=assessment_Conduct_repo.get_assessmentconduct_by_id(recordid=assessmentconductid)
