...
class AssessmentConductRepository:
    ...
    def create_assessmentconductcreate(self,candidate=None,assessment=None,assessment_date=None,currentuser=None):
        try:
            objassessmentconductdetails=self.model(assessment=assessment,candidate=candidate,assessment_date=assessment_date)
            objassessmentconductdetails.save(user=currentuser)
            
            return objassessmentconductdetails
        except Exception as Ex:
            raise RepositoryError(message="AssessmentConductRepository: Failed to create assessment conduct:", original_exception=Ex)       
    ...
    def check_assessmentconduct(self,candidate=None,assessment=None,assessment_date=None,currentuser=None):
        try:
            ...
            assessmentdateframe_min=datetime.strptime(yesterday_Date.strftime("%Y-%m-%d"),"%Y-%m-%d").date()
            assessmentdateframe_max=datetime.strptime(date.today().strftime("%Y-%m-%d"),"%Y-%m-%d").date()
        
            qs =  AssessmentConduct.objects.filter(active=True,assessment_end_datetime=None,type_of_closure=None,assessment_date__gte=assessmentdateframe_min,assessment_date__lte=assessmentdateframe_max)
            if candidate:
            ...
            raise RepositoryError(message="AssessmentConductRepository: Failed to get assessments filtered:", original_exception=Ex)
        
    
    def auto_close_assignedassessment(self, assessmentconduct=None):
        try:
            #print(type(assessment_data))
            aassessmentconduct_edit=AssessmentConduct.objects.get(id=assessmentconduct.id) 
            with transaction.atomic():                
...
candidate_mailid=None):
    ...
    def get_assessmentconduct_by_id(self,recordid):
        try:
            ...
            raise RepositoryError(message="AssessmentConductRepository: Failed to get assessment conduct by recordid:", original_exception=Ex)
    ...
    def update_assessementconduct_start_time(self, recordid=None):
        try:
            ...
            raise RepositoryError(message="AssessmentConductRepository Repository: Failed to update assessment:", original_exception=Ex)
