...
from apps.assessment_admin.adminforms.assessment_form import QuestionCreateEditForm,AssessmentSearchForm
...
class CreateQuestionView(View):
    ...
    def log_form_failure(self,form):
        ...
        ViewError(message="CreateQuestionView: Form Validation Failed:", original_exception=None,extra=errordetails,log=True)
    ...
    def get(self,request,*args,**kwargs):
        ...
        try:
            ...
            selected_assessmentid_decoded = signing.loads(selected_assessmentid, salt='assessment-salt', max_age=28800)
            if selected_assessmentid_decoded:
                    selected_assessmentid_decoded=selected_assessmentid_decoded['id'] 
            current_assessment=self._service.get_assessment_by_id(recordid=selected_assessmentid_decoded)
            self.Question_report_list=self._service.read_question_by_assessment(assessment=current_assessment)
            if self.Question_report_list:
                for question in self.Question_report_list:
                    question.signed_id=signing.dumps({'id':question.id},salt='assessment-salt')
                paginator=Paginator(self.Question_report_list,self.paginate_by)
                page_number=request.GET.get('page',1)
                page_obj=paginator.get_page(page_number)
        
        except Exception as Ex:
                ViewError(message="CreateQuestionView: Failed on get method:", original_exception=Ex,log=True)
                messages.error(request, "Page Error Please Escalate")

        context={
            'pagetitle':'Add/Edit Question','current_assessment':current_assessment,'CreateQuestionForm':_QuestionCreateform,'page_obj':page_obj,
                    'form_card_class':self.form_card_class,"form_card_title":self.form_card_title,'signed_id':selected_assessmentid
                                    }
        ...
        if current_assessment:
            ...
            if ActionName and ActionName=="editquestion":
                try:
                    ...
                    selected_questionid_decoded = signing.loads(selected_questionid, salt='assessment-salt', max_age=28800)
                    _question=self._service.get_question_by_id(recordid=selected_questionid_decoded['id'])
                    if _question:
                        ...
                        _QuestionCreateform = self.form_class(initial={'question_id':selected_questionid,'question_text':_question.text,
                        ...
                        ViewError(message="CreateQuestionView: Failed to update question:", original_exception=Ex,extra=None,log=True)
                ...
                if form.is_valid():
                    try:
                        ...
                        _question=self._service.get_question_by_id(recordid=selected_questionid_decoded['id'])          
                        if _question:
                            form.cleaned_data['question_id']=selected_questionid_decoded['id']
                            question_result = self._service.update_question_and_options(form.cleaned_data,_question,currentuser=request.user)
                            if question_result:
                                ...
                                for i,option in enumerate(_option_list[:4]):
                                    if option.is_correct:
                                        correct_index=str(i)
                                        break
                                _QuestionCreateform = self.form_class(initial={'question_id':selected_questionid,'question_text':question_result.text,
                ...
                selected_questionid=request.POST.get("questionidforaction")
                selected_questionid_decoded = signing.loads(selected_questionid, salt='assessment-salt', max_age=28800)
                delete_question=self._service.get_question_by_id(recordid=selected_questionid_decoded['id'])          
                
                try:                 
                    if delete_question:
                        delete_status = self._service.update_question_remove(delete_question,currentuser=request.user)
                        if delete_status:
                    ...
                    ViewError(message="CreateQuestionView: Failed to delete question:", original_exception=Ex,extra=None,log=True)
                ...
                if form.is_valid():
                    try:
                        assessment = self._service.create_question(curr_assessment=current_assessment,cleaned_data=form.cleaned_data,currentuser=request.user)
                        if assessment:
                        ...
                        ViewError(message="CreateQuestionView: Failed to save question:", original_exception=Ex,extra=None,log=True)
                        messages.error(request, "Data Save Failed")
                else:
                    self.log_form_failure(form)
                    messages.error(request, "Invalid or Incomplete data filled")
                ...
                if srch_text:
                    try:
                        self.Question_report_list = self._service.search_question_bytext(assessment=current_assessment,srchtext=srch_text)
                        if not self.Question_report_list:
                            messages.warning(request, "No Data Found")
                    except Exception as Ex:
                        ViewError(message="CreateQuestionView: Failed to search question:", original_exception=Ex,extra=None,log=True)
                ...
                if form.is_valid():
                    try:
                        ...
                        ViewError(message="CreateQuestionView: Failed to reset page:", original_exception=Ex,extra=None,log=True)
