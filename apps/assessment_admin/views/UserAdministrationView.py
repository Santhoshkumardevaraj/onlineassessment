...
from apps.assessment_admin.adminforms.userprofile_form import UserProfileCreationForm,UserUpdateForm,UserProfileUpdateForm,UserProfileListForm
...
class CreateUserView(View):
    ...
    def get(self,request,*args,**kwargs):
        ...
        try:
            ...
            ViewError(message="CreateUserView: Error on get method:", original_exception=Ex,log=True)
            messages.error(request, "Page Error Please Escalate")

        context={
            'pagetitle':'Create User','UserCreateForm':_userform,'ProfileList':profile_list,'page_obj':page_obj
        }
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            try:
                user_profile = self._service.create_user_and_profile(form.cleaned_data,_currentuser)
                if user_profile:
                ...
                ViewError(message="CreateUserView: Error on save/create user:", original_exception=Ex,log=True)
                messages.error(request, "Data Save Failed")
        else:
            errordetails={'formerror':form.errors.as_json()}                 
            ViewError(message="CreateUserView: Form Validation Failed:", original_exception=None,extra=errordetails,log=True)
            ...
            try:
                ...
                if user:                
                    ...
                    if request.user.userprofile.role=="Admin":
                        ...
                        profile_form = self._userprofileupdateform(instance=user_profile)
                    else:
                ...
                ViewError(message="UpdateUserView: Error on get method:", original_exception=Ex,log=True)
                messages.error(request, "Data Save Failed")
    ...
    def post(self, request, *args, **kwargs):
        ...
        if user_form.is_valid() and profile_form.is_valid():
            try:
                ...
                ViewError(message="UpdateUserView: Error update user details:", original_exception=Ex,log=True)
                messages.error(request, "Page Error Please Escalate")
        else:           
            errordetails = {"user_form_errors": user_form.errors.as_json(),"profile_form_errors": profile_form.errors.as_json(),"username": request.GET.get("username"),"submitted_data": request.POST.dict()}
            ViewError(message="UpdateUserView: Form Validation Failed:", original_exception=None,extra=errordetails,log=True)
            messages.error(request, "Invalid or Incomplete data filled")
    ...
    def get(self,request,*args,**kwargs):
        ...
        try:            
            ...
            page_obj=paginator.get_page(page_number)
                ...
                ViewError(message="ListUserView: Error on get method:", original_exception=Ex,log=True)
                messages.error(request, "Page Error Please Escalate")
    ...
    def post(self,request,*args,**kwargs):        
        ...
        if post_form.is_valid():
            try:
                ...
                ProfileList=self._service.get_user_list_filtered(site=sitefilter,role=rolefilter,language=languagefilter)
            except Exception as Ex:
                ViewError(message="ListUserView: Failed on post method:", original_exception=Ex,log=True)
                messages.error(request, "Page Error Please Escalate")            
        else:
            errordetails={'formerror':post_form.errors.as_json()}                 
            ViewError(message="ListUserView: Form Validation Failed:", original_exception=None,extra=errordetails,log=True)
            messages.error(request, "Invalid or Incomplete data filled")
