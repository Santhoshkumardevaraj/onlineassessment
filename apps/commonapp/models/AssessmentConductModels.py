...
class AssessmentConduct(AuditFieldModel):
    ...
    def __str__(self):
        ...
        assessment_str = self.assessment.title if self.assessment else "Deleted Assessment"
    ...
    def save(self, *args, **kwargs):
        ...
        if self.assessment_start_datetime and self.assessment_end_datetime:
            assessment_starttime=self.assessment_start_datetime.strftime('%Y-%m-%d %H:%M:%S')
            assessment_endtime=self.assessment_end_datetime.strftime('%Y-%m-%d %H:%M:%S')
            assessment_starttime=datetime.strptime(assessment_starttime,'%Y-%m-%d %H:%M:%S')
            assessment_endtime=datetime.strptime(assessment_endtime,'%Y-%m-%d %H:%M:%S')
            #print(assessment_starttime,assessment_endtime)
            duration_in_seconds = (assessment_endtime - assessment_starttime).total_seconds()
