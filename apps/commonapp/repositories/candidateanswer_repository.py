...
from apps.commonapp.models.AssessmentModels import Assessment, Option, Question
from apps.commonapp.models.CandidateAnswerModel import CandidateAnswer


class CandidateAnswerRepository:

    def __init__(self, model_class=CandidateAnswer):
        self.model = model_class

    # candidate,assessment,question,assessment_assign, selected_option, is_correct, score

    def create_candidateanswer(
        self,
        assessmentconduct=None,
        candidate=None,
        assessment=None,
        questions=None,
        currentuser=None,
        ...
        try:
            ...
            with transaction.atomic():  # ensures all-or-nothing save
                for q in questions:
                    CandidateAnswer.objects.create(
                        ...
                        question=q,
                        assessmentconduct=assessmentconduct,
            ...
            raise RepositoryError(
                message="Assessment Repository: Failed to create assessment:",
                original_exception=Ex,
            )

    def read_candidate_questions(self, assessmentconduct=None):
        try:
            return self.model.objects.filter(
                active=True, assessmentconduct=assessmentconduct
    ...
    def candidate_questions_mark_read(self, recordid=None):
        try:
            Candidate_answer_edit = self.model.objects.get(id=recordid)
            Candidate_answer_edit.question_status = "viewed"
            Candidate_answer_edit.save()
    ...
    def get_candidateanswer_by_id(self, recordid):
        try:
            return self.model.objects.get(id=recordid)
        except Exception as Ex:
            raise RepositoryError(
                message="Assessment Repository: Failed to get assessment by recordid:",
                original_exception=Ex,
            )
    ...
    def create_question(
        ...
        try:
            ...
            question = Question.objects.create(
                ...
                text=Question_model_data["text"],
                question_type=Question_model_data["question_type"],
                created_by=currentuser,
            )
            ...
            raise RepositoryError(
                message="Assessment Repository: Failed to create questions and options:",
                original_exception=Ex,
            )
    ...
    def update_question_and_options(self, cleaned_data, question, currentuser):
        try:
            ...
            raise RepositoryError(
                message="Assessment Repository: Failed to update questions and options:",
                original_exception=Ex,
            )
    ...
    def read_question_by_assessment(self, assessment=None):
        try:
            ...
            List_Questions = Question.objects.filter(
                active=True, assessment=curr_assessment.id
            ).order_by("-created_datetime")

            return List_Questions
        except Exception as Ex:
            raise RepositoryError(
                message="Assessment Repository: Failed to read question by assessment:",
                original_exception=Ex,
            )
    ...
    def search_question_bytext(self, assessment=None, srchtext=None):
        try:
            ...
            List_Questions = Question.objects.filter(
                active=True, assessment=curr_assessment.id, text__icontains=srchtext
            ).order_by("-created_datetime")

            return List_Questions
        except Exception as Ex:
            raise RepositoryError(
                message="Assessment Repository: Failed to read question by hint:",
                original_exception=Ex,
            )
    ...
    def get_question_by_id(self, recordid):
        try:
            ...
            raise RepositoryError(
                message="Assessment Repository: Failed to read question by recordid:",
                original_exception=Ex,
            )
    ...
    def update_question_remove(self, question_data=None, currentuser=None):
        try:
            ...
            raise RepositoryError(
                message="Assessment Repository: Failed to remove question and option:",
                original_exception=Ex,
            )
        return None
