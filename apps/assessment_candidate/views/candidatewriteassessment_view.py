                    ...
                    if self.assigned_assessment:
                        candidate_answer_list = (
                            self._candidateanswer_service.read_candidate_questions(
                                self.assigned_assessment.id
                            )
                        )
                        if candidate_answer_list:
                            for candidate_answer in candidate_answer_list:
                                candidate_answer.signed_id = signing.dumps(
                                    {"id": candidate_answer.id},
                                    salt="candidateanswer-salt",
                                )
        ...
        if ActionName and ActionName == "startassessment":
            try:
                if self.assigned_assessment and self.assigned_assessment.assessment:
                    all_questions = list(
                        self.assigned_assessment.assessment.questions.all()
                    )
