...
from apps.assessment_candidate.services.assessment_service import AssessmentService
from apps.assessment_candidate.services.assessmentassign_service import AssessmentAssignService
from apps.assessment_candidate.services.useradministration_service import useradministrationservice
...
class CandidateWriteAssessment(View):
    template_name='candidate/WriteAssessment.html'
    ...
    previousquestionid=None
    nextquestionid=None

    def get_assessments_assigned_for_candidate(self,candidate_profile,currentuser):
        self.assigned_assessment=self._assessmentassignservice.check_assessmentconduct(candidate=candidate_profile,assessment=None,assessment_date=None,currentuser=currentuser).first()
        
        if self.assigned_assessment:            
            self.assigned_assessment.signed_id=signing.dumps({'id':self.assigned_assessment.id},salt='assessmentconduct-salt')
    ...
    def get(self,request,*args,**kwargs):
        ...
        try:            
            ...
            if cand_loginid and cand_loginid!='':
                ...
                if candidate_profile and candidate_profile.username:
                    ...
                    if self.assigned_assessment:
                        candidate_answer_list=self._candidateanswer_service.read_candidate_questions(self.assigned_assessment.id)
                        if candidate_answer_list:
                            for candidate_answer in candidate_answer_list:
                                candidate_answer.signed_id=signing.dumps({'id':candidate_answer.id},salt='candidateanswer-salt')
                            self.currentquestionid=candidate_answer_list.first().id
        ...
        context={
            'pagetitle':'Assign Assessment','AssessmentSearchForm':_searchform,'AssessmentList':self.assessment_report_list,'page_obj':page_obj,
                    ...
                    'assigned_assessment':self.assigned_assessment,'currentquestionid':self.currentquestionid
                                    }
        ...
        if ActionName and ActionName=="startassessment":
            try:
                if self.assigned_assessment and self.assigned_assessment.assessment:
                    all_questions=list(self.assigned_assessment.assessment.questions.all())
                    total_question_count=len(all_questions)
                    questions_tobegenerated_assessment=self.assigned_assessment.assessment.num_of_questions
                    
                    if total_question_count>0:
                        num_to_pick=min(questions_tobegenerated_assessment,total_question_count)
                        questions_to_be_assigned=random.sample(all_questions,num_to_pick)
                        if questions_to_be_assigned:
                            add_candidatequestions=self._candidateanswer.create_candidateanswer(candidate=candidate_profile,assessment=self.assigned_assessment.assessment,assessmentconduct=self.assigned_assessment,questions=questions_to_be_assigned,currentuser=request.user)
                ...
                ViewError(message="StartAssessmentView: Failed to edit assessment:", original_exception=Ex,log=True)
