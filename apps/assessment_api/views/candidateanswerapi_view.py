...
from apps.assessment_api.serializers.CandidateAnswerUpdate_serializer import \
    CandidateAnswerUpdateSerializer
...
from apps.assessment_api.services.candidateanswer_service import \
...
class CandidateAnswerView(APIView):
    ...
    def update_assessemnt_start_time(self, selected_assessmentconductid_decoded):
        assigned_assessment = (
            self._assessment_assign_service.get_assessmentconduct_by_id(
                recordid=selected_assessmentconductid_decoded
            )
        )
        if not assigned_assessment.assessment_start_datetime:
            self._assessment_assign_service.update_assessmentconduct_starttime(
    ...
    def get_time_remaining_inseconds(self, selected_assessmentconductid_decoded):
        ...
        if assigned_assessment and assigned_assessment.assessment_start_datetime:
            ...
            end_assessment_time = assessment_starttime + timedelta(
                minutes=allowed_duration_minutes
            )
    ...
    def get(self, request, *args, **kwargs):
        ...
        selected_assessmentconductid_decoded = signing.loads(
            selected_assessmentconductid, salt="assessmentconduct-salt", max_age=28800
        )
        if selected_assessmentconductid_decoded:
            selected_assessmentconductid_decoded = selected_assessmentconductid_decoded[
                "id"
            ]

            candidateanswer_list = (
                self._candidateanswer_service.read_candidate_questions(
                    assessmentconductid=selected_assessmentconductid_decoded
                )
            )
            if not candidateanswer_list:
                return Response(
                    {"result": "No Records Found"}, status=status.HTTP_204_NO_CONTENT
                )

            self.update_assessemnt_start_time(selected_assessmentconductid_decoded)
            time_remaining_inseconds = self.get_time_remaining_inseconds(
                selected_assessmentconductid_decoded
            )
            data = [
                {
                    "signed_id": signing.dumps(
                        {"id": candidate_answer.question.id},
                        salt="assessmentconduct-salt",
                    ),
                    "question_status": candidate_answer.question_status,
                }
                for candidate_answer in candidateanswer_list
            ]
            # Update assessment start time

            return Response(
                {"result": data, "time_remaining_inseconds": time_remaining_inseconds},
...
class CandidateAnswer_questionView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if currentquestionid:
            ...
            selected_currentquestionid_decoded = signing.loads(
                selected_currentquestionid, salt="assessmentconduct-salt", max_age=28800
            )
            if selected_currentquestionid_decoded:
                selected_currentquestionid_decoded = selected_currentquestionid_decoded[
                    "id"
                ]
                current_question_readed = self._assessment_service.get_question_by_id(
                    selected_currentquestionid_decoded
                )
                if current_question_readed:
                    current_question = current_question_readed.text
                    current_question_id = signing.dumps(
                        {"id": current_question_readed.id},
                        salt="assessmentconduct-salt",
                    )
                    current_options = current_question_readed.options.filter(
                        active=True
                    )
                    ...
                    if candidateanswer_list:
                        ...
                        if selected_currentquestionid_decoded in ids:
                            ...
                            if idx > 0:
                                ...
                                previous_question_id = signing.dumps(
                                    {"id": previous_id}, salt="assessmentconduct-salt"
                                )
                            else:
                                previous_question_id = None
                            ...
                            if idx < len(ids) - 1:
                                next_id = ids[idx + 1]
                                next_question_id = signing.dumps(
                                    {"id": next_id}, salt="assessmentconduct-salt"
                                )
                            else:
                                next_question_id = None

                            options = current_question_readed.options.filter(
                                active=True
                            )
                            ...
                            res = self._candidateanswer_service.candidate_questions_mark_read(
                                recordid=current_candidate_answer_id
                            )
                            candidate_answer_id = signing.dumps(
                                current_candidate_answer_id,
                                salt="assessmentconduct-salt",
                            )
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            if candidateanswerid:
                ...
                selected_candidateanswerid_decoded = signing.loads(
                    selected_candidateanswerid,
                    salt="assessmentconduct-salt",
                    max_age=28800,
                )
                ...
                if selected_candidateanswerid_decoded:
                    candidateanswer = (
                        self._candidateanswer_service.get_candidateanswer_by_id(
                            recordid=selected_candidateanswerid_decoded
                        )
                    )

                    serializer = CandidateAnswerUpdateSerializer(
                        candidateanswer,
                        data={"option_id": selected_candidateansweroptionid_decoded},
                        partial=True,
                    )
            ...
            ViewError(
                message="APIView Error: Failed to get data: ",
...
class CandidateEndAssessmentView(APIView):
    ...
    def update_assessemnt_end(
        self, selected_assessmentconductid_decoded, assessment_endtype
    ):
        assigned_assessment = (
            self._assessment_assign_service.get_assessmentconduct_by_id(
                recordid=selected_assessmentconductid_decoded
        ...
        if (
            assigned_assessment.assessment_start_datetime
            and not assigned_assessment.assessment_end_datetime
        ):
            self._assessment_assign_service.update_assessmentconduct_end(
                recordid=selected_assessmentconductid_decoded,
                type_of_closure=assessment_endtype,
            )
    ...
    def get(self, request, *args, **kwargs):
        ...
        if selected_assessmentconductid_decoded:
            ...
            self.update_assessemnt_end(
                selected_assessmentconductid_decoded, assessment_endtype
            )
            # Update assessment start time
