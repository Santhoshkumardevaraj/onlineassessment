...
class CreateEditLookupView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            ViewError(
                message="CreateEditLookupView: Failed in get method:",
        ...
        context = {
            ...
            "page_obj": page_obj,
            "TypeLookupForm": get_form,
            "sort": request.GET.get("sort", "created_datetime"),
        ...
        try:
            ...
            if request.POST.get("ActionName"):
                if actionname == "deletelookup" and deletelookupid:
                    delete_lookup = self._service.remove_typelookup(
                        lookup_id=deletelookupid, currentuser=_currentuser
                    )
                    if delete_lookup:
                    ...
                    if post_form.is_valid():
                        createlookup = self._service.create_lookup(
                            post_form.cleaned_data, currentuser=_currentuser
                        )
                        ...
                        if createlookup:
                            ...
                            messages.error(
                                request, "Error occured in save please escalate"
                            )
                    else:
                        errordetails = {"formerror": post_form.errors.as_json()}
                        ViewError(
                            message="CreateEditLookupView: Form Validation Failed:",
            ...
            ViewError(
                message="CreateEditLookupView: Failed to save data:",
                original_exception=Ex,
                extra=None,
                log=True,
            )
