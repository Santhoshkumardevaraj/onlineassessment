...
class AssessmentCreateEditForm(forms.ModelForm):
    id=forms.CharField(max_length=100,widget=forms.HiddenInput(attrs={'id':'hdfipusername','class':'form-control','name':'id'}))
    title=forms.CharField(label="Title",widget=forms.TextInput(attrs={'type':'text','id':'txtbxtitle','class':'form-control','placeholder':'Title'}),max_length=200,required=True)
    description =forms.CharField(label="Description",widget=forms.TextInput(attrs={'type':'text','id':'txtbxdescription','class':'form-control','placeholder':'Description'}),max_length=200,required=True)
    ...
    num_of_questions =forms.IntegerField(label="No of Questions",widget=forms.NumberInput(attrs={'id':'txtbxnofoquestions','class':'form-control','placeholder':'No of Questions','min':'0','max':'100'}),required=True)
    duration_minutes =forms.IntegerField(label="Duration in Minutes",widget=forms.NumberInput(attrs={'id':'txtbxdurationminutes','class':'form-control','placeholder':'Duration Minutes','min':'1','max':'120'}),required=True)
    
   
    class Meta:
        model=Assessment
        fields=['id','title','description','language','num_of_questions','duration_minutes']
...
class AssessmentSearchForm(forms.Form):   
    srchtitle=forms.CharField(label="Title",widget=forms.TextInput(attrs={'type':'text','id':'txtbxsrchtitle','class':'form-control','placeholder':'Title'}),max_length=200,required=False)   
    srchlanguage=forms.ChoiceField(label="Language",choices=[],widget=forms.Select(attrs={'id':'ddlsrchlanguage','name':'Language','class':'form-control'}),required=False)
    srchnum_of_questions =forms.IntegerField(label="No of Questions",widget=forms.NumberInput(attrs={'id':'txtbxsrchnofoquestions','class':'form-control','placeholder':'No of Questions','min':'0','max':'100'}),required=False)
    ...
    def _validate_alphanumeric(self, fieldname, value):
        ...
        if not re.fullmatch(ALLOWED_PATTERN, value):            
            FormError(message="Form Error: Validation:", original_exception=None,extra={"Error":fieldname+" must contain only alphanumeric characters."})
            raise forms.ValidationError("Invalid characters found: only letters, numbers, spaces and basic punctuation are allowed")
        return value
...
class QuestionCreateEditForm(forms.Form):
    question_id = forms.CharField(
        widget=forms.HiddenInput(attrs={"id":"HDFQuestionId","name":"Questionrecordidforedit"}),
        required=False
    )
    ...
    def clean(self):
        ...
        if correct_option is not None:
            ...
            if not selected_text or selected_text.strip() == "":
                self.add_error('correct_option', "The selected correct option cannot be empty.")
