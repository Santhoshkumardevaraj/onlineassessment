...
class useradministrationservice:
    ...
    def create_user_and_profile(self, cleaned_data, currentuser):
        try:
            ...
            # Business logic
            if role == "Candidate":
                last_user = self.user_repo.get_last_candidate_username(
                    loginprefix="CAND"
                )
                number = (
                    int(last_user.username.replace("CAND", "")) + 1
                    if last_user
                    else 100
                )
    ...
    def update_user_and_profile_password(
        self, user, default_password, candpassword, currentuser
    ):
        successflag = True
        try:
